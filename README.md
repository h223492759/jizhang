...
| **常用备注** | 预设常用消费名称，记账时点标签直接填；系统还会自动统计**高频**和**最近**用过的备注供点选，并带出对应分类/支付方式/金额 |
...
记账最烦的就是反复敲同样的备注。这里有三层：

| 分组 | 来源 | 说明 |
|---|---|---|
| **常用** | 手动预设 | 「常用备注」页添加，可绑定默认分类、支付方式、常用金额 |
| **高频** | 自动统计 | 同一备注在本账本用满 2 次就会出现，按次数排序 |
| **最近** | 自动统计 | 最近用过的备注，按时间倒序 |

在「记一笔」弹窗的备注框下方，这三组会显示成可点击的标签，点一下就填好备注，
并自动带出它上次用的分类和支付方式（不会覆盖你已经填过的内容）。

备注框右上角的「☆ 设为常用」可以把当前备注一键收藏；
在「常用备注」页点高频/最近标签上的 ☆ 也能收藏。
